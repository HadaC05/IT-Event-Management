...
final class SboAuthorization
{
    ...
    {
        $dateClause = $todayOnly
            ? " AND ((CURDATE() BETWEEN DATE(e.start_at) AND DATE(e.end_at) AND s.schedule_date = CURDATE()) OR e.start_at > CURRENT_TIMESTAMP)"
            : '';
        $statement = $this->db->prepare("SELECT sea.id,sea.officer_assignment_id,sea.session_code,r.code responsibility,
            ...
            a.id activity_id,a.name activity_name,a.event_schedule_id activity_schedule_id,t.id team_id,t.name team_name,t.color team_color,
        ...
        foreach ($rows as &$row) {
            ...
            $row['activity_schedule_id'] = $row['activity_schedule_id'] === null ? null : (int) $row['activity_schedule_id'];
            ...
            $row['assignment_state'] = $eventStart > $now ? 'upcoming' : ($eventEnd < $now ? 'ended' : 'current');
