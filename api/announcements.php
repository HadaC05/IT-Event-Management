...
final class AnnouncementRepository
{
    ...
    public function index(array $filters): array
    {
        ...
        return [
            ...
            'events' => $this->events(),
            'pending_posts' => (int) $this->scalar("SELECT COUNT(*) FROM posts WHERE is_official=0 AND status='pending' AND deleted_at IS NULL"),
            'summary' => [
                'published' => (int) $this->scalar("SELECT COUNT(*) FROM posts WHERE is_official=1 AND status='approved' AND deleted_at IS NULL"),
                'drafts' => (int) $this->scalar("SELECT COUNT(*) FROM posts WHERE is_official=1 AND status='draft' AND deleted_at IS NULL"),
                'events' => (int) $this->scalar("SELECT COUNT(DISTINCT event_id) FROM posts WHERE is_official=1 AND status='approved' AND event_id IS NOT NULL AND deleted_at IS NULL"),
                'archived' => (int) $this->scalar('SELECT COUNT(*) FROM posts WHERE is_official=1 AND deleted_at IS NOT NULL'),
            ],
    ...
    {
        $sql = 'SELECT * FROM posts WHERE id=? AND is_official=1'.($includeArchived ? '' : ' AND deleted_at IS NULL');
    ...
    private function optionalEventId(mixed $value): ?int
    {
        ...
        if (!ctype_digit((string) $value) || (int) $value < 1 || !(bool) $this->scalar('SELECT id FROM events WHERE id=? AND deleted_at IS NULL', [(int) $value])) {
