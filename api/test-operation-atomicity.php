...
try{
    ...
    $eligibleScoreTeams=(new AcademicPeriodScope($db))->teamIdsForEvent($event);
    $workspaceTeamIds=array_map(static fn(array $row):int=>(int)$row['id'],$scoreWorkspace['teams']);
    $assert(in_array($team,$workspaceTeamIds,true)&&count($workspaceTeamIds)===count($eligibleScoreTeams),'SBO scoring workspace exposes every eligible team');
    ...
    $scorePayload=['assignment_id'=>$scoringTask,'scores'=>array_fill_keys($eligibleScoreTeams,'50')];
    $expectedRawScoreMessage='Raw scores saved. The adviser will assign placement points when the competition is finalized.';
    $assert($officerScores->save($officerUser,$scorePayload)===$expectedRawScoreMessage,'SBO officer can save raw scores for every eligible team');
    $assert($officerScores->save($officerUser,$scorePayload)===$expectedRawScoreMessage,'identical raw-score retry is idempotent');
    $changedPayload=$scorePayload;$changedPayload['scores'][$team]='60';
    $assert($officerScores->save($officerUser,$changedPayload)===$expectedRawScoreMessage,'SBO officer can correct a raw score before adviser finalization');
    $savedRawScore=$db->prepare('SELECT raw_score FROM tbl_activity_raw_scores WHERE event_id=? AND activity_id=? AND team_id=?');$savedRawScore->execute([$event,$activity,$team]);
    $assert((float)$savedRawScore->fetchColumn()===60.0,'raw-score correction replaces only the assigned team entry');
