...
final class SboStudentsRepository{
 ...
 public function index(int $user,array $filters):array{$assignments=$this->assignments($user);$id=(int)($filters['assignment_id']??0);$selected=null;foreach($assignments as $row)if($row['id']===$id)$selected=$row;if(!$selected&&$assignments)$selected=$assignments[0];if(!$selected)return compact('assignments','selected')+['students'=>[]];
  ...
  $sql="SELECT DISTINCT u.id,u.id_number,u.first_name,u.middle_name,u.last_name,u.email,yl.label year_level,t.name team_name,tu.team_id,
   ...
   JOIN tbl_team_user tu ON tu.user_id=u.id JOIN tbl_teams t ON t.id=tu.team_id AND t.is_active=1 LEFT JOIN tbl_year_levels yl ON yl.id=u.year_level
   LEFT JOIN tbl_attendances ad ON ad.user_id=u.id AND ad.event_id=? AND ad.attendance_date=?
   LEFT JOIN tbl_attendance_entries ae ON ae.attendance_id=ad.id AND ae.event_schedule_id=? AND ae.session_code=? AND ae.activity_id=? AND ae.team_id=tu.team_id AND ae.phase='in' WHERE (?='general' OR tu.team_id=?)";
  $scope=$selected['responsibility']==='attendance'?$selected['scanner_mode']:'specific';
  $params=[$selected['event_id'],$selected['schedule_date'],$selected['event_schedule_id'],$selected['session_code'],$selected['activity_id'],$scope,$selected['scanner_team_id']];
  if($search!==''){$sql.=" AND (u.id_number LIKE ? OR u.first_name LIKE ? OR u.last_name LIKE ? OR CONCAT(u.first_name,' ',u.last_name) LIKE ?)";$term="%$search%";array_push($params,$term,$term,$term,$term);}if($status==='present')$sql.=' AND ae.id IS NOT NULL';if($status==='not_recorded')$sql.=' AND ae.id IS NULL';$sql.=' ORDER BY u.last_name,u.first_name';$s=$this->db->prepare($sql);$s->execute($params);$rows=$s->fetchAll();$students=[];foreach($rows as $row)if($this->auth->studentIsEligible($selected,(int)$row['id'],(int)$row['team_id'])){$row['id']=(int)$row['id'];$row['full_name']=trim(implode(' ',array_filter([$row['first_name'],$row['middle_name'],$row['last_name']])));$students[]=$row;}return compact('assignments','selected','students');}
}
