...
$tables = [
    'tbl_roles','tbl_user_statuses','tbl_users','tbl_teams','tbl_team_user','tbl_locations','tbl_events',
...
try {
    ...
    $db->exec("UPDATE tbl_events SET attendance_location_policy='off' WHERE id=1");
    $offDashboard = $scanner->dashboard(15, $assignment);
    $assert($offDashboard['selected_assignment']['location_policy'] === 'off',
        'SBO Officer receives the Adviser GPS Off policy immediately on the next request');
    $offScan = $scanner->scan(15, [
        'assignment_id'=>$assignment,'checkpoint'=>'in','mode'=>'qr','token'=>$secondCard['token'],
        'location'=>['unavailable_reason'=>'not_provided'],
    ]);
    $assert($offScan['checkpoint'] === 'in' && $offScan['location']['status'] === 'unavailable',
        'GPS Off allows a valid student QR scan from anywhere without officer GPS');
    $expect(static fn() => $scanner->scan(15, [
        'assignment_id'=>$assignment,'checkpoint'=>'in','mode'=>'qr','token'=>str_repeat('z',32),
        'location'=>['unavailable_reason'=>'not_provided'],
    ]), InvalidArgumentException::class, 'GPS Off still rejects a QR that was not generated by the system');
    ...
    $unavailableScans = array_values(array_filter($adviserView['scans'], static fn(array $scan): bool => $scan['location_status'] === 'unavailable'));
    $assert(count($adviserView['scans']) === 3 && count($insideScans) === 2 && count($unavailableScans) === 1,
        'SBO Adviser can review strict GPS scans and the GPS Off scan');
} finally {
