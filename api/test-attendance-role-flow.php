...
try {
    ...
    $loggedOfficerAssignment = (int) $db->query("SELECT officer_assignment_id FROM tbl_activity_logs WHERE action='sbo_attendance_scanned' ORDER BY id DESC LIMIT 1")->fetchColumn();
    $assert($assignment >= 100 && $loggedOfficerAssignment === 1,
        'Attendance audit uses the officer assignment rather than the event responsibility ID');
