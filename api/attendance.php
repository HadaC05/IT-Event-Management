...
{
    private const EVENT_PAGE_SIZE = 10;
    private const ROSTER_PAGE_SIZE = 50;
    private const STATUSES = ['present', 'late', 'absent', 'excused'];
    ...
    public function index(array $filters): array
    {
        ...
        $total = (int)$count->fetchColumn(); $lastPage=max(1,(int)ceil($total/self::EVENT_PAGE_SIZE)); $page=min($page,$lastPage);
        $sql = "SELECT e.id,e.title,e.location,e.start_at,e.end_at,e.audience_type,
            ...
            SUM(CASE WHEN a.status='excused' THEN 1 ELSE 0 END) excused_count
          FROM events e LEFT JOIN attendances a ON a.event_id=e.id $whereSql
        ...
        $statement->bindValue(':limit',self::EVENT_PAGE_SIZE,PDO::PARAM_INT);$statement->bindValue(':offset',($page-1)*self::EVENT_PAGE_SIZE,PDO::PARAM_INT);$statement->execute();
        $events=$statement->fetchAll();
        foreach($events as &$event){
            foreach(['id','attendances_count','attended_count','present_count','late_count','absent_count','excused_count'] as $key)$event[$key]=(int)$event[$key];
        ...
        $summary=$this->db->query("SELECT
          ...
          (SELECT COUNT(DISTINCT a.event_id) FROM attendances a JOIN events e ON e.id=a.event_id WHERE e.deleted_at IS NULL) tracked,
          (SELECT COUNT(*) FROM attendances a JOIN events e ON e.id=a.event_id WHERE e.deleted_at IS NULL) records,
          (SELECT COUNT(*) FROM attendances a JOIN events e ON e.id=a.event_id WHERE e.deleted_at IS NULL AND a.status IN ('present','late')) attended")->fetch();
        ...
        return ['events'=>$events,'summary'=>$summary,'pagination'=>['current_page'=>$page,'last_page'=>$lastPage,'total'=>$total,'from'=>$total?($page-1)*self::EVENT_PAGE_SIZE+1:null,'to'=>$total?min($page*self::EVENT_PAGE_SIZE,$total):null]];
    }

    public function roster(int $eventId,array $filters): array
    {
        $event=$this->event($eventId);$search=trim((string)($filters['search']??''));$status=trim((string)($filters['status']??''));$requestedDate=trim((string)($filters['date']??''));$page=max(1,(int)($filters['page']??1));
        if(mb_strlen($search)>100)throw new InvalidArgumentException('Search may not exceed 100 characters.');
        ...
        $expectedIds=$this->expectedIds($event);$recorded=$this->column('SELECT user_id FROM attendances WHERE event_id=? AND attendance_date=?',[$eventId,$date]);
        ...
        $statement=$this->db->prepare($sql);$allParams=array_merge([$eventId,$date],$params,[self::ROSTER_PAGE_SIZE,($page-1)*self::ROSTER_PAGE_SIZE]);
        ...
        $counts=array_fill_keys(self::STATUSES,0);$countsStatement=$this->db->prepare('SELECT status,COUNT(*) total FROM attendances WHERE event_id=? AND attendance_date=? GROUP BY status');$countsStatement->execute([$eventId,$date]);foreach($countsStatement as $row)if(isset($counts[$row['status']]))$counts[$row['status']]=(int)$row['total'];
        $recordedCount=array_sum($counts);$attended=$counts['present']+$counts['late'];$expected=count($expectedIds);
        $options=$this->db->query('SELECT id,title,start_at FROM events WHERE deleted_at IS NULL ORDER BY start_at DESC')->fetchAll();foreach($options as &$option)$option['id']=(int)$option['id'];unset($option);
        return ['event'=>$event,'event_options'=>$options,'attendance_dates'=>$dates,'attendance_date'=>$date,'participants'=>$participants,'participant_total'=>count($participantIds),'counts'=>$counts,'summary'=>['expected'=>$expected,'recorded'=>$recordedCount,'unrecorded'=>max(0,$expected-$recordedCount),'completion'=>$expected?min(100,round($recordedCount/$expected*100,1)):null,'attended'=>$attended,'rate'=>$recordedCount?round($attended/$recordedCount*100,1):null],'pagination'=>['current_page'=>$page,'last_page'=>$lastPage,'total'=>$total,'from'=>$total?($page-1)*self::ROSTER_PAGE_SIZE+1:null,'to'=>$total?min($page*self::ROSTER_PAGE_SIZE,$total):null]];
    }
    ...
    public function update(int $eventId,array $input,int $actorId): int
    {
        ...
        $recordedIds=array_map('intval',$this->column('SELECT user_id FROM attendances WHERE event_id=?',[$eventId]));
    ...
    private function event(int $id): array{$statement=$this->db->prepare('SELECT id,title,location,start_at,end_at,audience_type FROM events WHERE id=? AND deleted_at IS NULL');$statement->execute([$id]);$event=$statement->fetch();if(!$event)throw new InvalidArgumentException('Event not found.');$event['id']=(int)$event['id'];return $event;}
    private function scheduleDates(array $event): array{$dates=array_map('strval',$this->column('SELECT schedule_date FROM event_attendance_schedules WHERE event_id=? ORDER BY schedule_date',[$event['id']]));return $dates?:[substr((string)$event['start_at'],0,10)];}
    private function expectedIds(array $event): array
    {
        $base="SELECT DISTINCT u.id FROM users u JOIN roles r ON r.id=u.role_id JOIN user_statuses us ON us.id=u.status WHERE r.name='Student' AND us.label='active'";$params=[];
        if($event['audience_type']==='selected_tribes'){$base.=' AND EXISTS(SELECT 1 FROM team_user tu JOIN event_team et ON et.team_id=tu.team_id WHERE tu.user_id=u.id AND et.event_id=?)';$params[]=$event['id'];}
        elseif($event['audience_type']==='selected_year_levels'){$base.=' AND EXISTS(SELECT 1 FROM event_year_level eyl WHERE eyl.year_level_id=u.year_level AND eyl.event_id=?)';$params[]=$event['id'];}
        elseif($event['audience_type']==='specific_students'){$base.=' AND EXISTS(SELECT 1 FROM event_participants ep WHERE ep.user_id=u.id AND ep.event_id=?)';$params[]=$event['id'];}
