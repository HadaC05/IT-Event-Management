...
final class AttendanceManagementRepository
{
    ...
    public function roster(int $eventId,array $filters): array
    {
        ...
        elseif(in_array($status,self::STATUSES,true)){$where[]='EXISTS(SELECT 1 FROM tbl_attendances ax WHERE ax.user_id=u.id AND ax.event_id=? AND ax.attendance_date=? AND ax.status=?)';array_push($params,$eventId,$date,$status);}
        $whereSql=' WHERE '.implode(' AND ',$where);$count=$this->db->prepare('SELECT COUNT(*) FROM tbl_users u'.$whereSql);$count->execute($params);$total=(int)$count->fetchColumn();$lastPage=max(1,(int)ceil($total/self::ROSTER_PAGE_SIZE));$page=min($page,$lastPage);
        ...
        $counts=array_fill_keys(self::STATUSES,0);$countsStatement=$this->db->prepare('SELECT status,COUNT(*) total FROM tbl_attendances WHERE event_id=? AND attendance_date=? GROUP BY status');$countsStatement->execute([$eventId,$date]);foreach($countsStatement as $row)if(isset($counts[$row['status']]))$counts[$row['status']]=(int)$row['total'];
