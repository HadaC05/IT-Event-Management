...
final class ScoreManagementRepository
{
    ...
    public function show(int $eventId): array
    {
        ...
        $categories = $this->rows('SELECT c.*,COUNT(s.id) scores_count FROM tbl_score_categories c LEFT JOIN tbl_scores s ON s.score_category_id=c.id WHERE c.event_id=? GROUP BY c.id ORDER BY c.sort_order,c.id', [$eventId]);
        ...
        $scores = $this->rows('SELECT id,event_id,team_id,score_category_id,points FROM tbl_scores WHERE event_id=? AND score_category_id IS NOT NULL', [$eventId]);
    ...
    {
        $event=$this->event($eventId);$category=$this->category($eventId,$categoryId);$this->db->prepare('DELETE FROM tbl_score_categories WHERE id=?')->execute([$categoryId]);
    ...
    public function saveScores(int $eventId, array $input, int $actorId): int
    {
        ...
        $categoryRows=$this->rows('SELECT id,max_points FROM tbl_score_categories WHERE event_id=?',[$eventId]);$categories=[];foreach($categoryRows as $row)$categories[(int)$row['id']] = (float)$row['max_points'];
    ...
    private function category(int $eventId,int $id):array{$rows=$this->rows('SELECT * FROM tbl_score_categories WHERE id=? AND event_id=?',[$id,$eventId]);if(!$rows)throw new InvalidArgumentException('Scoring category not found.');return$rows[0];}
