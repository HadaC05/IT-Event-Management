...
final class ScoreManagementRepository
{
    ...
    public function show(int $eventId, ?int $activityId = null): array
    {
        ...
        $activities = $this->rows('SELECT ea.id,ea.event_id,a.label AS name,a.description,ea.status FROM tbl_event_activities ea INNER JOIN tbl_activities a ON a.id=ea.activity_id WHERE ea.event_id=? ORDER BY ea.status="active" DESC,ea.id', [$eventId]);
