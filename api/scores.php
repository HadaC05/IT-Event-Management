...
final class ScoreManagementRepository
{
    ...
    public function show(int $eventId, ?int $activityId = null): array
    {
        ...
        $activities = $this->rows('SELECT id,event_id,name,description,status FROM tbl_event_activities WHERE event_id=? ORDER BY status="active" DESC,id', [$eventId]);
        ...
        $teams = $this->rows("SELECT t.id,t.name,t.color,t.is_active,sy.label school_year_label,COALESCE(SUM(CASE WHEN s.event_id=? AND s.score_category_id IN (SELECT c.id FROM tbl_score_categories c WHERE c.event_id=? AND c.activity_id=?) THEN s.points ELSE 0 END),0) event_score_total
            FROM tbl_teams t LEFT JOIN tbl_school_years sy ON sy.id=t.school_year_id LEFT JOIN tbl_scores s ON s.team_id=t.id
            WHERE t.is_active=1 OR EXISTS(SELECT 1 FROM tbl_scores sx WHERE sx.team_id=t.id AND sx.event_id=?)
            GROUP BY t.id ORDER BY event_score_total DESC,t.name", [$eventId,$eventId,$activityId ?? 0,$eventId]);
        ...
        return ['event'=>$event,'activities'=>$activities,'selected_activity'=>$selectedActivity,'categories'=>$categories,'teams'=>$teams,'scores'=>$scoreMap,'event_options'=>$options,'summary'=>['categories'=>count($categories),'teams'=>count($teams),'entries'=>$entries,'completion'=>$possible?round($entries/$possible*100,1):null,'total_points'=>(float)$total,'maximum'=>(float)array_sum(array_column($categories,'max_points'))]];
    }

    public function createCategory(int $eventId, array $input, int $actorId): array
    {
        $event=$this->event($eventId);$activity=$this->activity($eventId,(int)($input['activity_id']??0));[$name,$maximum]=$this->validateCategory($eventId,$activity['id'],$input);
    ...
    {
        $event=$this->event($eventId);$activity=$this->activity($eventId,(int)($input['activity_id']??0));$category=$this->category($eventId,$categoryId,$activity['id']);[$name,$maximum]=$this->validateCategory($eventId,$activity['id'],$input,$categoryId);
        $highest=(float)$this->scalar('SELECT COALESCE(MAX(points),0) FROM tbl_scores WHERE score_category_id=?',[$categoryId]);
    ...
    {
        $event=$this->event($eventId);$activity=$this->activity($eventId,(int)($input['activity_id']??0));$category=$this->category($eventId,$categoryId,$activity['id']);$this->db->prepare('DELETE FROM tbl_score_categories WHERE id=?')->execute([$categoryId]);
    ...
    {
        $event=$this->event($eventId);$activity=$this->activity($eventId,(int)($input['activity_id']??0));$submitted=$input['scores']??null;if(!is_array($submitted))throw new InvalidArgumentException('Scores are required.');
        $categoryRows=$this->rows('SELECT id,max_points FROM tbl_score_categories WHERE event_id=? AND activity_id=?',[$eventId,$activity['id']]);$categories=[];foreach($categoryRows as $row)$categories[(int)$row['id']] = (float)$row['max_points'];
        $allowed=array_map('intval',$this->column('SELECT id FROM tbl_teams WHERE is_active=1 OR EXISTS(SELECT 1 FROM tbl_scores WHERE tbl_scores.team_id=tbl_teams.id AND tbl_scores.event_id=?)',[$eventId]));
