                        ...
                        <tbody class="divide-y divide-[#121017]/8">
                            @foreach($categories as $category)
                                <tr class="transition hover:bg-white/80">
                                    ...
                                    <td class="px-6 py-3.5"><div class="flex justify-end gap-2"><button class="min-h-10 rounded-xl border border-[#2F3AE0]/25 bg-[#2F3AE0]/8 px-4 text-xs font-black text-[#2F3AE0] transition hover:bg-[#2F3AE0]/14" type="button" data-dialog-open="category-{{ $category->id }}">Edit</button><form method="POST" action="{{ route('adviser.scores.categories.destroy', [$event, $category]) }}" data-confirm-title="Remove scoring category?" data-confirm-message="{{ $category->name }} and its {{ $category->scores_count }} score entries will be permanently removed." data-confirm-action="Remove category">@csrf @method('DELETE')<button class="min-h-10 rounded-xl border border-[#FF6B2C]/25 bg-[#FF6B2C]/9 px-4 text-xs font-black text-[#d9470a] transition hover:bg-[#FF6B2C]/15" type="submit" data-loading-text="Removing…">Remove</button></form></div></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
