...
@section('content')
    ...
    <section class="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,.65fr)]">
        ...
        <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <header class="flex items-end justify-between gap-4 border-b border-slate-100 px-6 py-5"><div><h2 class="text-lg font-extrabold tracking-tight text-[#121017]">Team Leaderboard</h2><p class="mt-1 text-xs text-slate-500">Overall team rankings</p></div><span class="text-right"><strong class="block text-sm text-[#121017]">{{ rtrim(rtrim(number_format($stats['points_awarded'], 2), '0'), '.') }}</strong><small class="text-xs text-slate-400">total points</small></span></header>
