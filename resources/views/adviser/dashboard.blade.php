...
@section('content')
    <header class="mb-8 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div>
            ...
            <h1 class="text-3xl font-extrabold tracking-tight text-[#141e46] sm:text-4xl">
    ...
    <section class="mb-5 grid gap-4 lg:grid-cols-3" aria-label="Primary dashboard indicators">
        <article class="relative overflow-hidden rounded-2xl bg-[#141e46] p-6 text-white shadow-lg shadow-slate-900/10">
        ...
        <article class="flex flex-col justify-between rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between"><p class="text-xs font-extrabold uppercase tracking-[.14em] text-slate-400">Students Present</p><span class="h-2.5 w-2.5 rounded-full bg-emerald-500" aria-hidden="true"></span></div>
            <div class="mt-6"><strong class="text-5xl font-black leading-none tracking-tight text-[#141e46]">{{ number_format($stats['students_present']) }}</strong><p class="mt-3 text-sm text-slate-500">of {{ number_format($stats['students']) }} registered students</p></div>
        </article>

        <article class="flex flex-col justify-between rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between"><p class="text-xs font-extrabold uppercase tracking-[.14em] text-slate-400">Upcoming Events</p><span class="h-2.5 w-2.5 rounded-full bg-[#141e46]" aria-hidden="true"></span></div>
            <div class="mt-6"><strong class="text-5xl font-black leading-none tracking-tight text-[#141e46]">{{ number_format($stats['upcoming_events']) }}</strong>
    ...
    <section class="mb-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" aria-label="People overview">
        <div class="grid divide-y divide-slate-100 sm:grid-cols-[1.2fr_repeat(3,1fr)] sm:divide-x sm:divide-y-0">
            <div class="p-5 sm:p-6"><p class="text-xs font-extrabold uppercase tracking-[.14em] text-emerald-700">People Overview</p><div class="mt-2 flex items-baseline gap-2"><strong class="text-3xl font-black text-[#141e46]">{{ number_format($stats['total_users']) }}</strong><span class="text-xs font-semibold text-slate-400">Total Users</span></div></div>
            <div class="flex items-center justify-between gap-4 px-5 py-4 sm:block sm:p-6"><span class="text-sm font-semibold text-slate-500">Students</span><strong class="text-2xl font-extrabold text-[#141e46] sm:mt-2 sm:block">{{ number_format($stats['students']) }}</strong></div>
            <div class="flex items-center justify-between gap-4 px-5 py-4 sm:block sm:p-6"><span class="text-sm font-semibold text-slate-500">Faculty</span><strong class="text-2xl font-extrabold text-[#141e46] sm:mt-2 sm:block">{{ number_format($stats['faculty']) }}</strong></div>
            <div class="flex items-center justify-between gap-4 px-5 py-4 sm:block sm:p-6"><span class="text-sm font-semibold text-slate-500">SBO</span><strong class="text-2xl font-extrabold text-[#141e46] sm:mt-2 sm:block">{{ number_format($stats['sbo']) }}</strong></div>
        </div>
    </section>
    ...
    <section class="mb-6 grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,.65fr)]">
        <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <header class="flex flex-wrap items-end justify-between gap-4 border-b border-slate-100 px-6 py-5">
                <div><h2 class="text-lg font-extrabold tracking-tight text-[#141e46]">Attendance Overview</h2><p class="mt-1 text-xs text-slate-500">Today’s attendance status distribution</p></div>
            ...
            @if ($todayAttendance['marked'] > 0)
                <div class="p-6">
                    <div class="h-3 overflow-hidden rounded-full bg-slate-100" role="progressbar" aria-label="Today's attendance rate" aria-valuemin="0" aria-valuemax="100" aria-valuenow="{{ $todayAttendance['rate'] }}"><div class="h-full rounded-full bg-emerald-500" style="width: {{ min(100, $todayAttendance['rate']) }}%"></div></div>
                    <div class="mt-6 grid grid-cols-2 gap-x-6 gap-y-5 sm:grid-cols-4">
                        <div><span class="mb-2 block h-1 w-8 rounded-full bg-emerald-500"></span><strong class="text-2xl font-extrabold text-[#141e46]">{{ number_format($todayAttendance['present']) }}</strong><p class="mt-1 text-xs font-semibold text-slate-500">Present</p></div>
                        <div><span class="mb-2 block h-1 w-8 rounded-full bg-amber-500"></span><strong class="text-2xl font-extrabold text-[#141e46]">{{ number_format($todayAttendance['late']) }}</strong><p class="mt-1 text-xs font-semibold text-slate-500">Late</p></div>
                        <div><span class="mb-2 block h-1 w-8 rounded-full bg-rose-500"></span><strong class="text-2xl font-extrabold text-[#141e46]">{{ number_format($todayAttendance['absent']) }}</strong><p class="mt-1 text-xs font-semibold text-slate-500">Absent</p></div>
                        <div><span class="mb-2 block h-1 w-8 rounded-full bg-slate-400"></span><strong class="text-2xl font-extrabold text-[#141e46]">{{ number_format($todayAttendance['excused']) }}</strong><p class="mt-1 text-xs font-semibold text-slate-500">Excused</p></div>
                    </div>
                </div>
            @else
        ...
        <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <header class="border-b border-slate-100 px-6 py-5"><h2 class="text-lg font-extrabold tracking-tight text-[#141e46]">Today’s Events</h2><p class="mt-1 text-xs text-slate-500">Schedule for {{ now()->format('F j') }}</p></header>
            @forelse ($todayEvents as $event)
                @php($stateTone = match($event->schedule_state) { 'ongoing' => 'bg-emerald-500', 'completed' => 'bg-slate-300', default => 'bg-[#141e46]' })
    ...
    <section class="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,.65fr)]">
        <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <header class="border-b border-slate-100 px-6 py-5"><h2 class="text-lg font-extrabold tracking-tight text-[#141e46]">Recent Attendance</h2><p class="mt-1 text-xs text-slate-500">Latest attendance activity across events</p></header>
            @if ($recentAttendances->isNotEmpty())
                <div class="overflow-x-auto"><table class="w-full min-w-[620px] text-left"><thead class="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-400"><tr><th class="px-6 py-3">Student</th><th class="px-4 py-3">Event</th><th class="px-4 py-3">Status</th><th class="px-6 py-3 text-right">Recorded</th></tr></thead><tbody class="divide-y divide-slate-100">
                    @foreach ($recentAttendances as $attendance)
                        @php($statusTone = match($attendance->status) { 'present' => 'bg-emerald-50 text-emerald-700', 'late' => 'bg-amber-50 text-amber-700', 'absent' => 'bg-rose-50 text-rose-700', default => 'bg-slate-100 text-slate-600' })
        ...
        <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <header class="flex items-end justify-between gap-4 border-b border-slate-100 px-6 py-5"><div><h2 class="text-lg font-extrabold tracking-tight text-[#141e46]">Team Leaderboard</h2><p class="mt-1 text-xs text-slate-500">Overall team rankings</p></div><span class="text-right"><strong class="block text-sm text-[#141e46]">{{ rtrim(rtrim(number_format($stats['points_awarded'], 2), '0'), '.') }}</strong><small class="text-xs text-slate-400">total points</small></span></header>
            @forelse ($leaderboard as $team)
                @php($rankTone = match($team->rank) { 1 => 'bg-amber-100 text-amber-800', 2 => 'bg-slate-200 text-slate-700', 3 => 'bg-orange-100 text-orange-800', default => 'bg-slate-100 text-slate-600' })
                <div class="flex items-center gap-4 border-b border-slate-100 px-6 py-4 last:border-b-0"><span class="grid h-9 w-9 shrink-0 place-items-center rounded-full text-xs font-extrabold {{ $rankTone }}" aria-label="Rank {{ $team->rank }}">{{ $team->rank }}</span><span class="grid min-w-0 flex-1 gap-1"><strong class="truncate text-sm text-slate-700">{{ $team->name }}</strong><small class="text-xs text-slate-400">{{ number_format($team->members_count) }} {{ Str::plural('member', $team->members_count) }}</small></span><strong class="text-lg font-extrabold text-[#141e46]">{{ rtrim(rtrim(number_format($team->scores_sum_points, 2), '0'), '.') }}</strong></div>
            @empty
