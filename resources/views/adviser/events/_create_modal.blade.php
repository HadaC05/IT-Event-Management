    ...
    <form method="POST" action="{{ route('adviser.events.store') }}" enctype="multipart/form-data" class="p-5 sm:p-7" data-create-event-form>
        ...
        <div class="grid gap-5">
            ...
            <section class="border-t border-slate-100 pt-5">
                ...
                @else
                    <label class="grid gap-2"><span class="sr-only">Event-in-Charge</span><select class="{{ $field }} {{ $errors->has('assigned_user_ids') ? $invalid : '' }}" name="assigned_user_ids[]" data-event-in-charge><option value="">Assign later</option>@foreach($assignableUsers as $user)<option value="{{ $user->id }}" @selected($selectedUsers->contains($user->id))>{{ $user->id === auth()->id() ? 'You — '.$user->role->name : $user->full_name.' — '.$user->role->name }}</option>@endforeach</select><x-form-error name="assigned_user_ids" /></label>
                @endif
            </section>
...
<script>
    ...
    const checkConflicts = () => {
        ...
        conflictTimer = window.setTimeout(async () => {
            ...
            const person = createEventForm.querySelector('[data-event-in-charge]')?.value;
            if (person) payload.append('assigned_user_ids[]', person);
            try {
