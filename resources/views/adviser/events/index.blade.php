...
@section('content')
    <header class="mb-8 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div><p class="mb-2 text-xs font-extrabold uppercase tracking-[.14em] text-emerald-700">Administration</p><h1 class="text-3xl font-extrabold tracking-tight text-[#141e46] sm:text-4xl">Event Management</h1><p class="mt-2 text-sm text-slate-500">Create, organize, and assign the people responsible for each event.</p></div>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <form class="flex flex-wrap items-center gap-2 border-b border-slate-100 p-4" method="GET" action="{{ route('adviser.events.index') }}">
            ...
            <label><span class="sr-only">Filter by schedule</span><select class="h-11 min-w-36 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-600 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10" name="timing"><option value="">Any schedule</option><option value="upcoming" @selected(request('timing') === 'upcoming')>Upcoming</option><option value="ongoing" @selected(request('timing') === 'ongoing')>Ongoing</option><option value="completed" @selected(request('timing') === 'completed')>Completed</option><option value="archived" @selected(request('timing') === 'archived')>Archived</option></select></label>
            ...
            @if(request()->hasAny(['search','status','timing']))<a class="px-2 py-3 text-xs font-bold text-emerald-700" href="{{ route('adviser.events.index') }}">Clear</a>@endif
        </form>
            ...
            <table class="w-full min-w-[850px] border-collapse">
                ...
                @forelse($events as $event)
                    <tr class="group transition {{ $event->trashed() ? 'bg-slate-50/70 opacity-75' : 'cursor-pointer hover:bg-emerald-50/40' }}" @unless($event->trashed()) data-row-url="{{ route('adviser.events.show',$event) }}" tabindex="0" role="link" aria-label="View {{ $event->title }} details" @endunless>
                        <td class="px-5 py-4"><div class="grid min-w-48 gap-1"><strong class="text-sm text-[#141e46] group-hover:text-emerald-700">{{ $event->title }}</strong><small class="max-w-sm truncate text-[10px] text-slate-400">{{ $event->description ?: 'No description' }}</small></div></td>
                        ...
                        <td class="px-4 py-4"><div class="flex flex-wrap gap-1.5"><span class="rounded-full px-2.5 py-1 text-[10px] font-bold {{ match($event->schedule_state) { 'ongoing'=>'bg-amber-50 text-amber-700','completed'=>'bg-slate-100 text-slate-600','archived'=>'bg-red-50 text-red-700',default=>'bg-emerald-50 text-emerald-700' } }}">{{ ucfirst($event->schedule_state) }}</span>@unless($event->trashed())<span class="rounded-full px-2.5 py-1 text-[10px] font-bold {{ $event->status?->label === 'inactive' ? 'bg-red-50 text-red-700' : 'bg-blue-50 text-blue-700' }}">{{ $event->status?->label === 'inactive' ? 'Deactivated' : 'Active' }}</span>@endunless</div></td>
                        ...
                        <td class="px-5 py-4 text-right"><div class="inline-flex gap-1.5">
                            ...
                            @else
                                <a class="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-500 hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700" href="{{ route('adviser.events.edit',$event) }}" aria-label="Edit {{ $event->title }}"><svg class="h-4 w-4 fill-none stroke-current" viewBox="0 0 24 24"><path d="M12 20h9M16.5 3.5a2.12 2.12 0 0 1 3 3L8 18l-4 1 1-4Z"/></svg></a>
                                <form method="POST" action="{{ route('adviser.events.destroy',$event) }}" data-confirm-title="Archive event?" data-confirm-message="{{ $event->title }} will be hidden, but its details and assignments will be preserved." data-confirm-action="Archive">@csrf @method('DELETE')<button class="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-500 hover:border-red-200 hover:bg-red-50 hover:text-red-600" type="submit" data-loading-text="Archiving…" aria-label="Archive {{ $event->title }}"><svg class="h-4 w-4 fill-none stroke-current" viewBox="0 0 24 24"><path d="M4 7h16v13H4V7Zm-1-4h18v4H3V3Zm6 8h6"/></svg></button></form>
                            @endif
                        </div></td>
                    </tr>
                @empty
                    <tr><td class="px-5 py-16 text-center" colspan="6"><strong class="text-sm text-slate-600">No events found</strong><p class="mt-1 text-xs text-slate-400">{{ request()->hasAny(['search','status','timing']) ? 'Try changing your filters.' : 'Create your first event to get started.' }}</p></td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
