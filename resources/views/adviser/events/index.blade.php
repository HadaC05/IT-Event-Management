            ...
            <table class="w-full min-w-[850px] border-collapse">
                ...
                @forelse($events as $event)
                    <tr class="group transition {{ $event->trashed() ? 'bg-slate-50/70 opacity-75' : 'cursor-pointer hover:bg-emerald-50/40' }}" @unless($event->trashed()) data-row-url="{{ route('adviser.events.show',$event) }}" tabindex="0" role="link" aria-label="View {{ $event->title }} details" @endunless>
                        ...
                        <td class="px-4 py-4"><div class="flex flex-wrap gap-1.5"><span class="rounded-full px-2.5 py-1 text-[10px] font-bold {{ match($event->schedule_state) { 'ongoing'=>'bg-[#C6F24E]/35 text-[#397565]','completed'=>'bg-[#121017]/6 text-[#121017]/55','archived'=>'bg-[#FF6B2C]/10 text-[#FF6B2C]',default=>'bg-[#2F3AE0]/8 text-[#2F3AE0]' } }}">{{ ucfirst($event->schedule_state) }}</span>@unless($event->trashed())<span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold {{ $event->status?->label === 'inactive' ? 'bg-[#FF6B2C]/10 text-[#FF6B2C]' : 'bg-[#C6F24E]/35 text-[#397565]' }}"><i class="h-1.5 w-1.5 rounded-full {{ $event->status?->label === 'inactive' ? 'bg-[#FF6B2C]' : 'bg-[#C6F24E]' }}"></i>{{ $event->status?->label === 'inactive' ? 'Deactivated' : 'Active' }}</span>@endunless</div></td>
