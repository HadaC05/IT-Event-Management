    ...
    <div class="grid grid-cols-1 gap-5 xl:grid-cols-[.8fr_1.2fr]">
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <dl class="px-6">
                ...
                <div class="grid grid-cols-[90px_1fr] gap-3 border-b border-slate-100 py-5"><dt class="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Officer scans</dt><dd>@if(count($event->attendanceSlots()) === 4)<div class="grid grid-cols-2 gap-2">@foreach($event->attendanceSlots() as $slot)<span class="rounded-lg bg-[#397565]/7 px-2.5 py-2 text-[9px] font-black text-[#397565]">{{ $slot['label'] }}<strong class="mt-0.5 block text-xs text-[#121017]">{{ $slot['at']->format('M j · g:i A') }}</strong></span>@endforeach</div>@else<span class="text-xs font-medium text-slate-400">Not configured</span>@endif</dd></div>
