...
@section('content')
    ...
    <header class="mb-8 flex items-start gap-4"><span class="mt-1 grid h-12 w-12 shrink-0 place-items-center rounded-xl text-sm font-black text-white shadow-sm" style="background-color: {{ $team->color }}">{{ strtoupper(substr($team->name, 0, 2)) }}</span><div><p class="mb-2 text-xs font-extrabold uppercase tracking-[.14em] text-emerald-700">Tribe Details</p><h1 class="text-3xl font-extrabold tracking-tight text-[#141e46] sm:text-4xl">Edit {{ $team->name }}</h1><p class="mt-2 text-sm text-slate-500">Update its identity, school year, or student roster.</p></div></header>
