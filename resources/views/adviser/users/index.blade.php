...
@section('content')
    ...
    <dialog class="m-auto max-h-[calc(100vh_-_2rem)] w-[min(760px,calc(100%_-_2rem))] overflow-y-auto rounded-2xl border-0 bg-white p-0 shadow-2xl backdrop:bg-[#121017]/60 backdrop:backdrop-blur-[2px]" id="add-user-dialog">
        <form method="POST" action="{{ route('adviser.users.store') }}">@csrf
            <header class="border-b border-slate-100 border-t-4 border-t-emerald-500 px-6 py-5"><div class="flex items-start justify-between gap-5"><div><p class="mb-2 text-xs font-extrabold uppercase tracking-[.14em] text-emerald-700">New Account</p><h2 class="text-2xl font-extrabold tracking-tight text-[#121017]">Add User</h2><p class="mt-1.5 text-sm text-slate-500">Create an SBO, Faculty, or Student account.</p></div><button class="grid h-9 w-9 place-items-center rounded-lg bg-slate-100 text-xl text-slate-500 hover:bg-slate-200" type="button" data-dialog-close aria-label="Close">&times;</button></div></header>
