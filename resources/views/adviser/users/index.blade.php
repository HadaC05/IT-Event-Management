...
@section('content')
    <header class="mb-8 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <h1 class="text-3xl font-extrabold tracking-tight text-[#121017] sm:text-4xl">User Management</h1>
    ...
    <section class="mb-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" aria-label="Account overview">
        <div class="grid divide-y divide-slate-100 md:grid-cols-[1.3fr_repeat(5,1fr)] md:divide-x md:divide-y-0">
            <div class="bg-[#121017] p-5 text-white md:p-6">
            ...
            @foreach ([['students', 'Students'], ['faculty', 'Faculty'], ['sbo', 'SBO Officers'], ['active', 'Active'], ['inactive', 'Inactive']] as [$key, $label])
                <div class="flex items-center justify-between gap-4 px-5 py-4 md:block md:p-6">
                    <span class="inline-flex items-center gap-2 text-sm font-semibold {{ $key === 'active' ? 'text-[#397565]' : ($key === 'inactive' ? 'text-[#FF6B2C]' : 'text-slate-500') }}">
                        @if (in_array($key, ['active', 'inactive']))<i class="h-2 w-2 rounded-full {{ $key === 'active' ? 'bg-[#C6F24E]' : 'bg-[#FF6B2C]' }}" aria-hidden="true"></i>@endif
                        {{ $label }}
                    </span>
                    <strong class="text-2xl font-extrabold text-[#121017] md:mt-2 md:block">{{ number_format($userSummary[$key]) }}</strong>
                </div>
            @endforeach
        </div>
    </section>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header class="border-b border-slate-100 px-5 py-5 sm:px-6">
            <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><h2 class="text-lg font-extrabold tracking-tight text-[#121017]">User Directory</h2><p class="mt-1 text-xs text-slate-500">{{ number_format($users->total()) }} {{ Str::plural('account', $users->total()) }} found</p></div>@if(request()->hasAny(['search', 'role', 'status']))<a class="mt-2 text-xs font-extrabold text-emerald-700 sm:mt-0" href="{{ route('adviser.users.index') }}">Clear all filters</a>@endif</div>
            ...
            <form class="mt-5 grid gap-2 md:grid-cols-[minmax(260px,1fr)_170px_170px_auto]" method="GET" action="{{ route('adviser.users.index') }}">
                ...
                <button class="inline-flex h-11 items-center justify-center rounded-xl bg-[#397565] px-5 text-sm font-extrabold text-white transition hover:bg-[#397565]/90" type="submit">Apply</button>
            </form>
        </header>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($users as $user)
                        @php
                            ...
                            $statusTone = match($statusLabel) { 'active' => 'text-[#397565]', 'inactive' => 'text-[#FF6B2C]', default => 'text-slate-500' };
                            $statusDot = match($statusLabel) { 'active' => 'bg-[#C6F24E]', 'inactive' => 'bg-[#FF6B2C]', default => 'bg-[#121017]/25' };
                        @endphp
                        <tr class="transition hover:bg-slate-50/70">
                            <td class="px-6 py-5"><div class="flex min-w-64 items-center gap-3"><span class="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#121017] text-xs font-extrabold text-[#F3F0E9]">{{ strtoupper(substr($user->full_name, 0, 1).substr(Str::afterLast($user->full_name, ' '), 0, 1)) }}</span><span class="grid min-w-0 gap-0.5"><strong class="truncate text-sm text-slate-700">{{ $user->full_name }}</strong><small class="truncate text-xs text-slate-400">{{ $user->display_email }}</small><small class="truncate text-xs text-slate-400">{{ '@'.$user->username }}@if($user->display_id_number) · {{ $user->display_id_number }}@endif</small></span></div></td>
                            ...
                            <td class="min-w-72 px-4 py-5">
                                ...
                                @elseif($user->role?->name === 'SBO Officer')
                                    <a class="inline-flex min-h-9 items-center gap-2 rounded-lg bg-[#397565]/8 px-3 text-xs font-extrabold text-[#397565]" href="{{ route('adviser.officers.index', ['search' => $user->display_id_number]) }}"><i class="h-2 w-2 rounded-full {{ $user->userStatus?->label === 'active' ? 'bg-[#C6F24E]' : 'bg-[#FF6B2C]' }}"></i>Manage officer assignment</a>
                            ...
                            <td class="px-6 py-5 text-right">
                                @if($manageable)
                                    <div class="inline-flex items-center gap-2"><button class="inline-flex min-h-10 items-center rounded-xl border border-[#397565]/25 bg-[#397565]/8 px-4 text-xs font-extrabold text-[#397565] hover:border-[#397565]/40 hover:bg-[#397565]/14" type="button" data-dialog-open="edit-{{ $user->id }}">Edit</button><form method="POST" action="{{ route('adviser.users.status', $user) }}" @if($user->userStatus?->label !== 'inactive') data-confirm-title="Deactivate user?" data-confirm-message="{{ $user->full_name }} will no longer be able to sign in until the account is activated again." data-confirm-action="Deactivate" @endif>@csrf @method('PATCH')<button class="inline-flex min-h-10 items-center rounded-xl border px-4 text-xs font-extrabold {{ $user->userStatus?->label === 'inactive' ? 'border-[#397565]/25 bg-[#C6F24E]/30 text-[#397565] hover:bg-[#C6F24E]/45' : 'border-[#FF6B2C]/25 bg-[#FF6B2C]/9 text-[#d9470a] hover:bg-[#FF6B2C]/15' }}" type="submit" data-loading-text="Updating…">{{ $user->userStatus?->label === 'inactive' ? 'Activate' : 'Deactivate' }}</button></form></div>
                                @endif
                            </td>
                        </tr>
                    @empty
        ...
        <div class="divide-y divide-slate-100 lg:hidden">
            @forelse($users as $user)
                ...
                @endphp
                <article class="p-5">
                    <div class="flex items-start gap-3"><span class="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#121017] text-xs font-extrabold text-[#F3F0E9]">{{ strtoupper(substr($user->first_name, 0, 1).substr($user->last_name, 0, 1)) }}</span><div class="min-w-0 flex-1"><strong class="block truncate text-sm text-slate-700">{{ $user->full_name }}</strong><p class="mt-0.5 truncate text-xs text-slate-400">{{ $user->email }}</p><div class="mt-2 flex flex-wrap items-center gap-2"><span class="rounded-lg border border-slate-200 px-2 py-1 text-xs font-bold text-slate-600">{{ $user->role?->name ?? 'Unassigned' }}</span><span class="inline-flex items-center gap-1.5 text-xs font-bold {{ $statusTone }}"><i class="h-2 w-2 rounded-full {{ $statusDot }}"></i>{{ ucfirst($statusLabel ?? 'Unassigned') }}</span></div></div></div>
                    @if(in_array($user->role?->name, ['SBO', 'Faculty']))<div class="mt-4 flex flex-wrap gap-2">@foreach($user->assignedEvents as $event)<span class="inline-flex items-center gap-1 rounded-lg bg-emerald-50 py-1 pl-2.5 pr-1 text-xs font-bold text-emerald-800" data-assignment-item>{{ $event->title }}<form method="POST" action="{{ route('adviser.users.events.destroy', [$user, $event]) }}" data-async-unassign>@csrf @method('DELETE')<button class="grid h-6 w-6 place-items-center rounded text-base text-emerald-700 hover:bg-rose-50 hover:text-rose-600" type="submit" aria-label="Unassign {{ $user->full_name }} from {{ $event->title }}">&times;</button></form></span>@endforeach<button class="rounded-lg px-2.5 py-1.5 text-xs font-extrabold text-emerald-700" type="button" data-dialog-open="assign-{{ $user->id }}">+ Assign Event</button></div>@elseif($user->role?->name === 'SBO Officer')<a class="mt-4 inline-flex min-h-9 items-center rounded-lg bg-[#397565]/8 px-3 text-xs font-extrabold text-[#397565]" href="{{ route('adviser.officers.index', ['search' => $user->display_id_number]) }}">Manage officer assignment</a>@endif
                    @if($manageable)<div class="mt-4 flex gap-2 border-t border-slate-100 pt-4"><button class="inline-flex min-h-11 flex-1 items-center justify-center rounded-xl border border-[#397565]/25 bg-[#397565]/8 text-xs font-extrabold text-[#397565]" type="button" data-dialog-open="edit-{{ $user->id }}">Edit account</button><form class="flex-1" method="POST" action="{{ route('adviser.users.status', $user) }}" @if($user->userStatus?->label !== 'inactive') data-confirm-title="Deactivate user?" data-confirm-message="{{ $user->full_name }} will no longer be able to sign in until the account is activated again." data-confirm-action="Deactivate" @endif>@csrf @method('PATCH')<button class="min-h-11 w-full rounded-xl border text-xs font-extrabold {{ $user->userStatus?->label === 'inactive' ? 'border-[#397565]/25 bg-[#C6F24E]/30 text-[#397565]' : 'border-[#FF6B2C]/25 bg-[#FF6B2C]/9 text-[#d9470a]' }}" type="submit" data-loading-text="Updating…">{{ $user->userStatus?->label === 'inactive' ? 'Activate' : 'Deactivate' }}</button></form></div>@endif
                </article>
            @empty
    ...
    <dialog class="m-auto max-h-[calc(100vh_-_2rem)] w-[min(760px,calc(100%_-_2rem))] overflow-y-auto rounded-2xl border-0 bg-white p-0 shadow-2xl backdrop:bg-[#121017]/60 backdrop:backdrop-blur-[2px]" id="add-user-dialog">
        <form method="POST" action="{{ route('adviser.users.store') }}">@csrf
            <header class="border-b border-slate-100 border-t-4 border-t-emerald-500 px-6 py-5"><div class="flex items-start justify-between gap-5"><div><p class="mb-2 text-xs font-extrabold uppercase tracking-[.14em] text-emerald-700">New Account</p><h2 class="text-2xl font-extrabold tracking-tight text-[#121017]">Add User</h2><p class="mt-1.5 text-sm text-slate-500">Create an SBO Adviser, Faculty, or Student account. Officers are assigned from existing students.</p></div><button class="grid h-9 w-9 place-items-center rounded-lg bg-slate-100 text-xl text-slate-500 hover:bg-slate-200" type="button" data-dialog-close aria-label="Close">&times;</button></div></header>
    ...
    @foreach($users as $user)
        @if(in_array($user->role?->name, ['SBO', 'Faculty', 'Student']))
            <dialog class="m-auto max-h-[calc(100vh_-_2rem)] w-[min(760px,calc(100%_-_2rem))] overflow-y-auto rounded-2xl border-0 bg-white p-0 shadow-2xl backdrop:bg-[#121017]/60 backdrop:backdrop-blur-[2px]" id="edit-{{ $user->id }}"><form method="POST" action="{{ route('adviser.users.update', $user) }}">@csrf @method('PUT')<header class="border-b border-slate-100 border-t-4 border-t-emerald-500 px-6 py-5"><div class="flex items-start justify-between gap-5"><div><p class="mb-2 text-xs font-extrabold uppercase tracking-[.14em] text-emerald-700">Account Details</p><h2 class="text-2xl font-extrabold tracking-tight text-[#121017]">Edit User</h2><p class="mt-1.5 text-sm text-slate-500">Update {{ $user->full_name }}’s information and access.</p></div><button class="grid h-9 w-9 place-items-center rounded-lg bg-slate-100 text-xl text-slate-500 hover:bg-slate-200" type="button" data-dialog-close aria-label="Close">&times;</button></div></header><div class="p-6">@include('adviser.users._form', ['editingUser' => $user])</div><footer class="flex justify-end gap-2 border-t border-slate-100 bg-slate-50/60 px-6 py-4"><button class="inline-flex min-h-10 items-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-600" type="button" data-dialog-close>Cancel</button><button class="inline-flex min-h-10 items-center rounded-xl bg-emerald-600 px-5 text-sm font-bold text-white" type="submit" data-loading-text="Saving…">Save Changes</button></footer></form></dialog>
        @endif
        @if(in_array($user->role?->name, ['SBO', 'Faculty']))
            <dialog class="m-auto w-[min(520px,calc(100%_-_2rem))] rounded-2xl border-0 bg-white p-0 shadow-2xl backdrop:bg-[#121017]/60 backdrop:backdrop-blur-[2px]" id="assign-{{ $user->id }}"><form method="POST" action="{{ route('adviser.users.events.store', $user) }}">@csrf<input type="hidden" name="_form" value="assign-{{ $user->id }}"><header class="border-b border-slate-100 border-t-4 border-t-emerald-500 px-6 py-5"><div class="flex items-start justify-between gap-5"><div><p class="mb-2 text-xs font-extrabold uppercase tracking-[.14em] text-emerald-700">Event Responsibility</p><h2 class="text-2xl font-extrabold tracking-tight text-[#121017]">Assign Event</h2><p class="mt-1.5 text-sm text-slate-500">Choose an event for {{ $user->full_name }}.</p></div><button class="grid h-9 w-9 place-items-center rounded-lg bg-slate-100 text-xl text-slate-500 hover:bg-slate-200" type="button" data-dialog-close aria-label="Close">&times;</button></div></header><div class="p-6">
