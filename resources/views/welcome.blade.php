...
<head>
    ...
    <style>
        ...
        body { position:relative; isolation:isolate; overflow-x:hidden; background:var(--bone); color:var(--ink); }
        body::before {
            content:"";
            position:fixed;
            inset:-90px;
            ...
            pointer-events:none;
            background-image:
                radial-gradient(circle at 12% 18%,rgba(57,117,101,.09),transparent 25%),
                radial-gradient(circle at 88% 68%,rgba(47,58,224,.055),transparent 22%),
                linear-gradient(rgba(57,117,101,.055) 1px,transparent 1px),
                linear-gradient(90deg,rgba(57,117,101,.055) 1px,transparent 1px);
...
<body class="min-h-screen font-sans antialiased selection:bg-[#C6F24E] selection:text-[#121017]">
    ...
    <header class="relative z-30 border-b border-[#121017]/10 bg-[#F3F0E9]/95 backdrop-blur">
        <nav class="mx-auto flex h-[72px] max-w-[1440px] items-center gap-5 px-5 sm:px-8 lg:px-12" aria-label="Main navigation">
            ...
            <div class="relative ml-auto">
                ...
                <div id="nav-menu" class="absolute right-0 top-[calc(100%+.75rem)] z-50 w-72 rounded-xl border border-[#121017]/10 bg-[#F3F0E9]/95 p-3 shadow-[0_20px_50px_rgba(18,16,23,.16)] backdrop-blur-xl" hidden>
                    <label class="relative mb-2 block lg:hidden">
                        <span class="sr-only">Search events</span>
            ...
            @auth
                <a class="inline-flex h-11 shrink-0 items-center gap-2 rounded-lg bg-[#397565] px-4 text-sm font-extrabold text-white shadow-[0_7px_18px_rgba(57,117,101,.22)] transition hover:-translate-y-0.5 hover:bg-[#2e6355]" href="{{ route('dashboard') }}">Dashboard <span aria-hidden="true">→</span></a>
            @else
                <button class="inline-flex h-11 shrink-0 items-center gap-2 rounded-lg bg-[#397565] px-4 text-sm font-extrabold text-white shadow-[0_7px_18px_rgba(57,117,101,.22)] transition hover:-translate-y-0.5 hover:bg-[#2e6355]" type="button" data-login-open>Sign in <span aria-hidden="true">→</span></button>
            @endauth
        </nav>
    </header>
    ...
    @guest
        <dialog id="login-modal" class="login-dialog m-auto w-[calc(100%-2rem)] max-w-[460px] overflow-hidden rounded-2xl border-0 bg-[#F3F0E9] p-0 text-[#121017] shadow-[0_30px_90px_rgba(18,16,23,.35)]">
            <div class="relative p-6 sm:p-9">
                ...
                <form class="mt-7 grid gap-5" method="POST" action="{{ route('login') }}">
                    @csrf
                    ...
                    <button class="inline-flex min-h-13 items-center justify-center gap-2 rounded-xl bg-[#397565] px-5 font-extrabold text-white shadow-[0_9px_24px_rgba(57,117,101,.22)] transition hover:bg-[#2e6355]" type="submit">Sign in <span class="text-xl" aria-hidden="true">→</span></button>
                </form>
