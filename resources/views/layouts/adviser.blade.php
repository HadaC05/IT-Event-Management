...
<body class="min-h-screen @yield('body-class', 'bg-slate-50') font-sans text-slate-700 antialiased">
    <aside class="fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full flex-col bg-[#141e46] px-5 pb-5 pt-7 text-[#fff5e0] shadow-2xl transition-transform duration-200 lg:translate-x-0 lg:shadow-none" id="sidebar">
        ...
        <nav class="grid gap-2" aria-label="Main navigation">
            ...
            <a href="{{ route('adviser.attendance.index') }}" @class(['flex min-h-12 items-center gap-3 rounded-xl px-4 text-sm font-bold transition', 'bg-[#C6F24E] text-[#121017] shadow-lg shadow-black/10' => request()->routeIs('adviser.attendance.*'), 'text-[#fff5e0]/65 hover:bg-white/5 hover:text-[#fff5e0]' => !request()->routeIs('adviser.attendance.*')])>
