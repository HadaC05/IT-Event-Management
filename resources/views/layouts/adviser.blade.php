...
<body class="app-shell min-h-screen @yield('body-class', 'bg-white') font-sans text-[#121017] antialiased">
    ...
    <div class="min-h-screen lg:ml-64">
        @php
            $adminSearch = match (true) {
                request()->routeIs('adviser.users.*') => [route('adviser.users.index'), 'Search users…'],
                request()->routeIs('adviser.teams.*') => [route('adviser.teams.index'), 'Search tribes…'],
                request()->routeIs('adviser.events.*') => [route('adviser.events.index'), 'Search events…'],
                request()->routeIs('adviser.attendance.show') => [url()->current(), 'Search this roster…'],
                request()->routeIs('adviser.attendance.*') => [route('adviser.attendance.index'), 'Search attendance events…'],
                request()->routeIs('adviser.scores.*') => [route('adviser.scores.index'), 'Search scoring events…'],
                default => [route('adviser.events.index'), 'Search events…'],
            };
        @endphp
        <header class="sticky top-0 z-30 border-b border-[#121017]/8 bg-white/95 backdrop-blur-xl">
            <div class="mx-auto flex h-[72px] w-full max-w-[1500px] items-center gap-3 px-4 sm:px-6 lg:px-10 xl:px-16">
                ...
                <form class="relative min-w-0 max-w-xl flex-1" method="GET" action="{{ $adminSearch[0] }}" role="search">
                    ...
                    <label class="sr-only" for="admin-search">{{ $adminSearch[1] }}</label>
                    <input class="h-11 w-full rounded-xl border border-[#121017]/9 bg-[#F3F0E9]/55 pl-10 pr-10 text-sm text-[#121017] outline-none transition placeholder:text-[#121017]/35 focus:border-[#397565]/45 focus:bg-white focus:ring-4 focus:ring-[#397565]/8" id="admin-search" name="search" type="search" value="{{ request('search') }}" placeholder="{{ $adminSearch[1] }}">
                    <button class="absolute right-1.5 top-1/2 grid h-8 w-8 -translate-y-1/2 place-items-center rounded-lg text-[#397565] transition hover:bg-[#397565]/10" type="submit" aria-label="Search"><span aria-hidden="true">→</span></button>
                </form>
        ...
        <main class="mx-auto w-full max-w-[1500px] px-4 py-7 sm:px-6 lg:px-10 lg:py-9 xl:px-16">@yield('content')</main>
    </div>
