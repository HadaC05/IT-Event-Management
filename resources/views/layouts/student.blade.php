...
<aside class="fixed inset-y-0 left-0 z-50 hidden w-20 flex-col border-r border-[#397565]/15 bg-[#E8F0EC] shadow-[8px_0_30px_rgba(18,16,23,.07)] transition-[width] duration-200 lg:flex" data-student-sidebar aria-label="Desktop student navigation">
    <div class="flex h-[68px] shrink-0 items-center gap-3 border-b border-[#397565]/15 px-3" data-sidebar-header>
        <span class="hidden flex-1 whitespace-nowrap text-center text-xs font-black uppercase tracking-[.12em] text-[#397565]" data-sidebar-label>
            Student Menu
        </span>
        <button class="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-white/70 text-[#397565] shadow-sm hover:bg-white" type="button" data-student-sidebar-toggle aria-controls="student-desktop-nav" aria-expanded="false" aria-label="Expand navigation">
    ...
    <nav class="grid gap-2 px-3 py-5" id="student-desktop-nav">
        @foreach($studentNavItems as [$route,$label,$path])
            <a href="{{ route($route) }}" title="{{ $label }}" @class(['group flex min-h-14 items-center gap-4 overflow-hidden rounded-2xl px-3 text-sm font-black transition','bg-[#397565] text-white shadow-lg shadow-[#397565]/20'=>request()->routeIs($route),'bg-white/55 text-[#52635E] shadow-sm hover:bg-white hover:text-[#397565]'=>!request()->routeIs($route)]) aria-label="{{ $label }}" @if(request()->routeIs($route)) aria-current="page" @endif>
    ...
    <div class="mx-3 mt-auto mb-5 hidden rounded-2xl bg-[#F7F4ED] p-3" data-sidebar-label><span class="block text-[9px] font-black uppercase tracking-wider text-[#397565]">Signed in as</span><strong class="mt-1 block truncate text-xs">{{ auth()->user()->full_name }}</strong></div>
</aside>
...
<header class="sticky top-0 z-40 border-b border-[#121017]/8 bg-white/90 backdrop-blur-xl transition-[margin] duration-200 lg:ml-20" data-student-sidebar-content>
    <nav class="flex h-[68px] w-full items-center pl-3 pr-2 sm:pl-4 sm:pr-3 lg:pl-4 lg:pr-4" aria-label="Student header">
        ...
        <details class="group relative ml-auto" data-account-menu>
            <summary class="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-full p-1 pr-2 transition hover:bg-[#397565]/7"><x-student-avatar :user="auth()->user()" size="h-9 w-9"/><span class="hidden max-w-32 truncate text-xs font-black sm:block">{{ auth()->user()->first_name }}</span><svg class="h-4 w-4 fill-none stroke-current stroke-2 transition group-open:rotate-180" viewBox="0 0 24 24" aria-hidden="true"><path d="m7 10 5 5 5-5"/></svg></summary>
...
<nav class="fixed inset-x-0 bottom-0 z-40 border-t border-[#397565]/15 bg-[#E8F0EC]/95 pb-[env(safe-area-inset-bottom)] shadow-[0_-12px_35px_rgba(18,16,23,.09)] backdrop-blur-xl lg:hidden" aria-label="Student pages">
    <div class="mx-auto grid h-[76px] max-w-3xl grid-cols-5 gap-1 px-1.5 py-1.5">
        @foreach($studentNavItems as [$route,$label,$path])<a href="{{ route($route) }}" @class(['group flex min-h-11 flex-col items-center justify-center gap-1 rounded-xl text-[10px] font-black transition focus-visible:outline-offset-[-2px]','bg-white/85 text-[#397565] shadow-sm ring-1 ring-[#397565]/10'=>request()->routeIs($route),'text-[#52635E] hover:bg-white/65 hover:text-[#397565]'=>!request()->routeIs($route)]) aria-label="{{ $label }}" @if(request()->routeIs($route)) aria-current="page" @endif><svg class="h-5 w-5 fill-none stroke-current stroke-[2.1] group-active:scale-90" viewBox="0 0 24 24" aria-hidden="true"><path d="{{ $path }}"/></svg><span>{{ $label }}</span></a>@endforeach
    </div>
</nav>
<x-notifications />
@stack('scripts')
