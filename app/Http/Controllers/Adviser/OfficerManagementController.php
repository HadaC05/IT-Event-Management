...
class OfficerManagementController extends Controller
{
    ...
    {
        $students = User::query()
            ->with(['yearLevel', 'teams.schoolYear'])
        ...
        $assignments = SboOfficerAssignment::query()
            ->with(['officerAccount.userStatus', 'team', 'assignedBy'])
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $student = User::with('role')->findOrFail($data['student_user_id']);
        abort_unless($student->role?->name === 'Student' && $student->id_number, 422, 'Select an existing student account.');
        $existingOfficer = User::where('id_number', $student->id_number)
        ...
        DB::transaction(function () use ($request, $student, $data) {
            ...
            $duplicate = SboOfficerAssignment::where('student_id', $student->id_number)
            ...
            $officer = User::where('id_number', $student->id_number)->where('role_id', $officerRole->id)->lockForUpdate()->first();
            $accountData = [
                'id_number' => $student->id_number, 'first_name' => $student->first_name, 'middle_name' => $student->middle_name,
                'last_name' => $student->last_name, 'email' => $student->email, 'year_level' => $student->year_level,
            ...
            ActivityLog::create(['actor_id' => $request->user()->id, 'subject_user_id' => $officer->id, 'student_id' => $student->id_number, 'officer_assignment_id' => $assignment->id, 'action' => 'officer_assigned', 'acting_role' => $request->user()->role?->name, 'description' => "{$student->full_name} was assigned as {$assignment->position} for {$assignment->term}."]);
        });
    ...
    private function endAssignments(Request $request, $ids): int
    {
        return DB::transaction(function () use ($request, $ids) {
            ...
            foreach ($assignments as $assignment) {
                ...
                ActivityLog::create(['actor_id' => $request->user()->id, 'subject_user_id' => $assignment->officer_user_id, 'student_id' => $assignment->student_id, 'officer_assignment_id' => $assignment->id, 'action' => 'officer_unassigned', 'acting_role' => $request->user()->role?->name, 'description' => "Officer assignment {$assignment->position} ({$assignment->term}) was ended."]);
            }
