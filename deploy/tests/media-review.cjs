...
let pinned = false;
const published = {id: 99, user_id: 9, event_id: null, content: 'Approved story', image_path: null, video_path: null,
  created_at: '2026-09-23 08:00:00', status: 'approved', is_official: false, event_title: null,
  author_name: 'Test Student', author_initials: 'TS', author_role: 'Student', viewer_reaction: null,
...
(async () => {
  ...
  try {
    for (const width of [360, 1280]) {
      ...
      await page.route('**/api/**', route => {
        ...
        if (url.pathname.endsWith('/media.php')) {
          if (route.request().method() === 'POST') {
            ...
            if (input.action === 'post_pin') pinned = Boolean(input.pin);
          ...
          } else payload = {success: true, data: {viewer: {id: 2, role: 'SBO Adviser', full_name: 'SBO Adviser'}, permissions: {moderate: true, manage_carousel: true, hide: true}, featured_events: [], active_events: [], post_events: [], carousel_events: [], event_program: [], own_posts: [], posts: [{...published, is_pinned: pinned}]}};
        }
      ...
      const publishedCard = page.locator('[data-public-feed] [data-post-id="99"]');
      await publishedCard.locator('[data-post-menu] summary').click();
      await publishedCard.locator('[data-pin-post]').click();
      await page.waitForFunction(() => document.querySelector('[data-public-feed] [data-post-id="99"]')?.textContent.includes('Pinned'));
      ...
      console.log(`PASS: adviser review queue, pin toggle, media preview, paging, draft preservation and layout at ${width}px`);
      await context.close();
