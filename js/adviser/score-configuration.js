window.SharedNavigation.ready.then(() => {
  ...
  const ruleRow = (placement = "", points = "") => `<div class="grid gap-3" style="grid-template-columns:minmax(0,1fr) minmax(0,1fr) 90px" data-rule-row><label><span class="sr-only">Place</span><input class="h-10 w-full rounded-lg border border-[#121017]/12 px-3 text-sm font-bold" type="number" min="1" max="255" step="1" inputmode="numeric" data-rule-placement value="${placement}" placeholder="Place" required></label><label><span class="sr-only">Points</span><input class="h-10 w-full rounded-lg border border-[#121017]/12 px-3 text-sm font-bold" type="number" min="0" max="255" step="1" inputmode="numeric" data-rule-points value="${points}" placeholder="Points" required></label><button class="flex min-h-10 items-center justify-center gap-2 rounded-lg border border-[#FF6B2C]/25 text-xs font-black text-[#D64A12]" type="button" data-remove-rule aria-label="Remove place" title="Remove place"><svg class="h-4 w-4 fill-none stroke-current stroke-2" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M10 11v6m4-6v6M9 7l1-2h4l1 2m-8 0 1 13h8l1-13"/></svg><span>Remove</span></button></div>`;
