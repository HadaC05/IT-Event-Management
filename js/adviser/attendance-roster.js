  ...
  function renderRows() {
    ...
    data.participants.forEach((person) => {
      const status = person.attendance_status || "",
        sourceNote = person.manual_status
          ? `<span class="mt-1 inline-flex rounded-full bg-[#FF6B2C]/10 px-2 py-0.5 text-[8px] font-black uppercase tracking-wide text-[#b94312]">Manual correction${person.has_scan_evidence ? " over scan" : ""}</span>`
          : person.has_scan_evidence
            ? '<span class="mt-1 inline-flex rounded-full bg-[#397565]/10 px-2 py-0.5 text-[8px] font-black uppercase tracking-wide text-[#397565]">Verified scan</span>'
            : "",
