(() => {
    ...
    const refreshHistory = async () => {
        ...
        if (summary.total > 0) {
            ...
            rate.textContent = `${summary.rate}% attendance`;
        } else {
        ...
        count.classList.toggle('hidden', records.length === 0);
        document.querySelector('[data-attendance-records]').innerHTML = records.length
            ? records.map(record).join('')
            : '<div class="px-6 py-10 text-center"><span class="mx-auto grid h-10 w-10 place-items-center rounded-full bg-[#397565]/8 text-[#397565]">✓</span><h3 class="mt-3 font-black">No records yet</h3><p class="mt-1 text-sm text-[#121017]/45">Completed attendance appears here.</p></div>';
    };
