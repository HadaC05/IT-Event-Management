    ...
    root.innerHTML = `
        ...
        <div class="student-profile-summary relative px-5 pb-5 sm:px-8 sm:pb-7">
          <div class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div class="student-profile-identity flex min-w-0 flex-col sm:flex-row sm:items-end sm:gap-5">
              ...
              <div class="min-w-0 pt-2 sm:pb-2"><div class="flex flex-wrap items-center gap-2"><h1 class="min-w-0 truncate text-2xl font-black tracking-tight sm:text-3xl">${esc(profile.full_name)}</h1>${CiteMediaPermissions.specialTagMarkup(profile.special_tag)}</div><p class="mt-1 text-sm font-bold text-[#121017]/50">${esc(profile.year_level_label || 'Student')}${profile.team_name ? ` · ${esc(profile.team_name)}` : ''}</p><p class="mt-1 text-xs text-[#121017]/38">${totalPosts} post${totalPosts === 1 ? '' : 's'}</p></div>
            </div>
      ...
      <dialog class="student-profile-dialog" data-profile-details-dialog aria-labelledby="student-profile-details-title">
        ...
        <div class="student-profile-dialog-body">
          <div class="student-profile-dialog-person"><span class="student-profile-dialog-avatar">${avatar}</span><div class="min-w-0"><strong>${esc(profile.full_name)}</strong>${CiteMediaPermissions.specialTagMarkup(profile.special_tag)}<span>${esc(profile.year_level_label || 'Student')}${profile.team_name ? ` · ${esc(profile.team_name)}` : ''}</span></div></div>
