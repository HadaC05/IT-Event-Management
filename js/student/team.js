    ...
    const teamPage = team => {
        ...
        return `
            <header class="student-team-hero ${team.image_path ? 'student-team-hero--has-image' : ''}" data-initial="${escapeHtml(team.name.charAt(0).toUpperCase())}">
                ...
                <div><span class="student-team-hero__badge" aria-label="${escapeHtml(team.name)} tribe logo">${team.image_path ? `<img src="${escapeHtml(team.image_path)}" alt="">` : escapeHtml(team.name.charAt(0).toUpperCase())}</span><p class="student-stage__eyebrow">Your people / ${escapeHtml(team.school_year)}</p><h1>${escapeHtml(team.name)}</h1><p class="mt-4 text-sm leading-6 text-white/75">${team.members_count} members. One team. Every moment counts.</p><p class="student-team-hero__color"><span aria-hidden="true"></span>Your team color</p></div>
