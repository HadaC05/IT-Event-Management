(() => {
  ...
  async function profile() {
    const data=await get();
    host.innerHTML=card(`<form class="grid grid-cols-1 gap-4 sm:grid-cols-2" data-profile>${[['first_name','First name'],['middle_name','Middle name'],['last_name','Last name'],['email','Email']].map(([key,label])=>`<label class="grid min-w-0 gap-1 text-sm font-bold">${label}<input class="min-h-11 min-w-0 rounded-xl border px-3 font-normal" name="${key}" value="${esc(data[key]||'')}" ${key==='email'?'type="email"':''} ${key==='first_name'||key==='last_name'?'required':''}></label>`).join('')}<label class="grid min-w-0 gap-1 text-sm font-bold sm:col-span-2">Bio<textarea class="min-h-28 min-w-0 rounded-xl border p-3 font-normal" name="bio" maxlength="280">${esc(data.bio||'')}</textarea></label><label class="grid min-w-0 gap-1 text-sm font-bold">Profile picture<input class="min-w-0 max-w-full" type="file" name="photo" accept="image/jpeg,image/png,image/webp"></label>${data.profile_photo_path?`<img class="h-24 w-24 rounded-full object-cover" src="${esc(data.profile_photo_path)}" alt="Profile picture">`:''}<div class="sm:col-span-2"><button class="rounded-xl bg-[#397565] px-5 py-3 text-sm font-bold text-white">Save profile</button><p class="mt-2 text-xs text-[#121017]/50">Role and assigned team are managed by the SBO Adviser.</p></div></form>`);
