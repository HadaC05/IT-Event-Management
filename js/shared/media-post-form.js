  ...
  function mount(host, context) {
    ...
    host.innerHTML = `<section class="rounded-xl border border-[#397565]/15 bg-white p-4 shadow-[0_12px_35px_rgba(18,16,23,.05)] sm:p-5">
      <form enctype="multipart/form-data" data-shared-post-form>
        ...
        <div class="cite-media-preview mt-3 hidden overflow-hidden rounded-xl border border-[#121017]/10 bg-[#121017]" data-media-preview><button class="cite-media-preview-zoom hidden" type="button" data-preview-open-image aria-label="View selected photo full size"><img alt="Selected image preview"></button><video class="hidden" controls playsinline></video></div>
    ...
    const preview = host.querySelector('[data-media-preview]'), imageButton = preview.querySelector('[data-preview-open-image]'), image = imageButton.querySelector('img'), video = preview.querySelector('video'), fileName = host.querySelector('[data-file-name]');
    let previewUrl = '';
    let editingPost = null;
    const clearPreview = (markRemoval = false) => { if(previewUrl) URL.revokeObjectURL(previewUrl); previewUrl=''; image.removeAttribute('src'); video.removeAttribute('src'); imageButton.classList.add('hidden'); video.classList.add('hidden'); preview.classList.add('hidden'); imageInput.value=''; videoInput.value=''; fileName.textContent=''; host.querySelector('[data-remove-media]').classList.add('hidden'); if(markRemoval) form.elements.remove_media.value='1'; };
    const showExistingMedia = post => {
      const type = post.image_path ? 'image' : post.video_path ? 'video' : null;
      if (!type) return;
      preview.classList.remove('hidden');
      host.querySelector('[data-remove-media]').classList.remove('hidden');
      fileName.textContent = type === 'image' ? 'Current photo · Choose Photo to replace' : 'Current video · Choose Video to replace';
      if (type === 'image') { image.src = post.image_path; image.alt = 'Current post photo'; imageButton.classList.remove('hidden'); }
      else { video.src = post.video_path; video.classList.remove('hidden'); }
    };
    const showFile = (input, type) => { const file=input.files?.[0]; if(!file)return; const max=type==='image'?5*1024*1024:25*1024*1024; if(file.size>max){const wasRemoved=form.elements.remove_media.value==='1';clearPreview();if(!wasRemoved && editingPost)showExistingMedia(editingPost);form.elements.remove_media.value=wasRemoved?'1':'0';notify('error',type==='image'?'Choose an image no larger than 5 MB.':'Choose a video no larger than 25 MB.');return;} const other=type==='image'?videoInput:imageInput; other.value=''; if(previewUrl)URL.revokeObjectURL(previewUrl);previewUrl='';image.removeAttribute('src');video.removeAttribute('src');imageButton.classList.add('hidden');video.classList.add('hidden'); form.elements.remove_media.value='0';previewUrl=URL.createObjectURL(file); fileName.textContent=file.name; preview.classList.remove('hidden'); host.querySelector('[data-remove-media]').classList.remove('hidden'); if(type==='image'){image.src=previewUrl;image.alt='Selected image preview';imageButton.classList.remove('hidden');}else{video.src=previewUrl;video.classList.remove('hidden');} };
    imageInput.onchange=()=>showFile(imageInput,'image'); videoInput.onchange=()=>showFile(videoInput,'video');
    imageButton.onclick=()=>window.CiteMediaPostCard.openImage(image.getAttribute('src'), image.alt);
    host.querySelector('[data-remove-media]').onclick=()=>clearPreview(true);
    const reset = () => { form.reset(); clearPreview(); editingPost=null; form.elements.action.value='create'; form.elements.id.value=''; form.elements.remove_media.value='0'; host.querySelector('[data-form-title]').textContent='Create a post'; host.querySelector('[data-submit]').textContent='Post'; host.querySelector('[data-cancel-edit]').classList.add('hidden'); host.querySelector('[data-edit-context]').classList.add('hidden'); if(homeMarker.isConnected)homeMarker.after(host); host.classList.remove('col-span-full','scroll-mt-24'); };
