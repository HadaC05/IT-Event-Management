(() => {
  ...
  const videoDuration = file => new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => finish(new Error('Video duration could not be read. Choose another video.')), 12000);
    const finish = result => {
      clearTimeout(timer);
    ...
    probe.onloadedmetadata = () => finish(Number.isFinite(probe.duration) && probe.duration > 0 ? probe.duration : new Error('Video duration could not be read. Choose another video.'));
    probe.onerror = () => finish(new Error('This video could not be opened. Choose another video.'));
    probe.src = url;
  });
  ...
  function mount(host, context) {
    ...
    let videoUrl = null, videoSelectionRequest = 0, checkingVideo = false;
    ...
    const render = () => {
      ...
      photos.forEach((item, index) => {
        ...
        img.src = item.url; img.alt = `Post photo ${index + 1}`; img.loading = 'lazy'; img.decoding = 'async';
        open.append(img);
    ...
    imageInput.onchange = () => {
      videoSelectionRequest++; checkingVideo = false;
    ...
    videoInput.onchange = async () => {
      ...
      videoInput.value = '';
      if (!file) return;
      const request = ++videoSelectionRequest;
      if (file.size > 25 * 1024 * 1024 || !['video/mp4', 'video/webm', 'video/quicktime'].includes(file.type)) {
        checkingVideo = false; render(); notify('error', 'Choose an MP4, WebM or QuickTime video no larger than 25 MB.'); return;
      }
      checkingVideo = true;
      fileName.textContent = 'Checking video length…';
      try {
        const duration = await videoDuration(file);
        if (request !== videoSelectionRequest) return;
        if (duration > MAX_VIDEO_SECONDS) { notify('error', 'Videos must be 5 minutes or shorter.'); render(); return; }
      } catch (error) {
        if (request === videoSelectionRequest) { notify('error', error.message || 'Video duration could not be read.'); render(); }
        return;
      } finally { if (request === videoSelectionRequest) checkingVideo = false; }
