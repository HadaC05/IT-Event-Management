(() => {
  ...
  function mount(host, context) {
    ...
    const showExistingMedia = post => {
      ...
      if (images.length) {
        fileName.textContent = `${images.length} current photo${images.length === 1 ? '' : 's'} · Choose Photos to replace`;
        images.forEach((path, index) => { const image = document.createElement('img'); image.className = 'h-24 w-full rounded-lg object-cover'; image.src = path; image.alt = `Current post photo ${index + 1}`; imageList.append(image); });
      } else {
    ...
    const showImages = () => {
      ...
      if (files.length > 10 || files.some(file => file.size > 5 * 1024 * 1024) || files.reduce((size, file) => size + file.size, 0) > 25 * 1024 * 1024) { imageInput.value = ''; notify('error', files.length > 10 ? 'Choose up to 10 photos per post.' : files.some(file => file.size > 5 * 1024 * 1024) ? 'Each photo must be no larger than 5 MB.' : 'The combined photo upload may not exceed 25 MB.'); return; }
      videoInput.value = ''; releasePreviewUrls(); imageList.replaceChildren(); video.classList.add('hidden'); video.removeAttribute('src');
      files.forEach((file, index) => { const image = document.createElement('img'); const url = URL.createObjectURL(file); previewUrls.push(url); image.className = 'h-24 w-full rounded-lg object-cover'; image.src = url; image.alt = `Selected photo ${index + 1}`; imageList.append(image); });
      form.elements.remove_media.value = '0'; fileName.textContent = `${files.length} photo${files.length === 1 ? '' : 's'} selected`; preview.classList.remove('hidden'); host.querySelector('[data-remove-media]').classList.remove('hidden');
    };
