(() => {
  ...
  function mount(host, context) {
    ...
    const showImages = () => {
      ...
      if (files.length > 30 || files.some(file => file.size > 5 * 1024 * 1024)) { imageInput.value = ''; notify('error', files.length > 30 ? 'Choose up to 30 photos per post.' : 'Each photo must be no larger than 5 MB.'); return; }
