  ...
  function mount(host, context) {
    ...
    const showExistingMedia = post => {
      ...
      if (images.length) {
        ...
        images.forEach((path, index) => appendImagePreview(path, `Current post photo ${index + 1}`));
      } else {
    ...
    const showImages = () => {
      ...
      files.forEach((file, index) => { const url = URL.createObjectURL(file); previewUrls.push(url); appendImagePreview(url, `Selected photo ${index + 1}`); });
