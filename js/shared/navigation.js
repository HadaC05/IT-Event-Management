(() => {
  ...
  async function mount() {
    ...
    ensureStyles();
    const [session, fragmentResponse] = await Promise.all([axios.get('api/auth.php?action=session'), axios.get('pages/shared/navigation.html?v=20260924-media-nav-dev-1',{responseType:'text'})]);
    if (autoRole) {
    ...
    appNotifications(header,csrf).catch(error => console.warn('Notifications could not be loaded.', error));
