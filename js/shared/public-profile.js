(() => {
  ...
  const profileMarkup = user => {
    ...
    const posts = Number(user.post_count) > 0
      ? `<section class="mt-8"><h2 class="mb-4 text-lg font-black">Public posts</h2><div class="grid gap-5" data-profile-posts></div><button class="${nextCursor ? '' : 'hidden'} mt-5 min-h-11 w-full rounded-xl border border-[#397565]/25 bg-white px-5 text-sm font-black text-[#397565]" type="button" data-profile-more>Load more posts</button></section>`
      : '<section class="mt-8 rounded-2xl border border-dashed border-[#397565]/20 bg-white px-6 py-10 text-center"><h2 class="font-black">No public posts yet</h2><p class="mt-1 text-sm text-[#121017]/55">You can still view this person’s profile.</p></section>';
    return `<section class="overflow-hidden rounded-2xl border border-[#397565]/15 bg-white shadow-sm">
      <div class="cite-public-profile-cover" style="--team-cover:${teamColor}">${user.team_name ? `<span class="cite-public-profile-team">${esc(user.team_name)} team color</span>` : ''}</div>
      <div class="relative flex flex-col items-center gap-4 px-5 pb-6 text-center sm:flex-row sm:items-start sm:px-7 sm:text-left">
        <span class="-mt-12 grid h-24 w-24 shrink-0 place-items-center overflow-hidden rounded-full border-4 border-white bg-[#C6F24E]/35 text-2xl font-black text-[#397565] shadow-sm sm:-mt-12">${user.profile_photo_path ? `<img class="h-full w-full object-cover" src="${esc(user.profile_photo_path)}" alt="${esc(user.full_name)} profile photo">` : esc(user.initials)}</span>
        <div class="min-w-0 pt-1"><h1 class="text-2xl font-black tracking-tight">${esc(user.full_name)}</h1><p class="mt-1 text-sm font-bold text-[#397565]">${esc(CiteMediaPermissions.roleLabel(user.role_name))}${teamMeta ? ` · ${teamMeta}` : ''}</p>${user.bio ? `<p class="mt-3 whitespace-pre-wrap text-sm leading-6 text-[#121017]/60">${esc(user.bio)}</p>` : ''}</div>
      </div>
    </section>${posts}`;
  };
