(() => {
  ...
  function ownPostsMarkup(posts, viewer) {
    ...
    return posts.map(post=>{
      ...
      const photos=images.length?`<div class="cite-post-gallery cite-own-post-gallery ${layout}" aria-label="Photos attached to this ${esc(post.status)} post">${visible.map((path,index)=>{const more=remaining>0&&index===3;return `<button class="cite-post-gallery-item" type="button" data-own-image-post="${post.id}" data-own-image-index="${more?4:index}" aria-label="${more?`View ${remaining} more photos`:`View photo ${index+1} of ${images.length}`}"><img src="${esc(path)}" alt="Photo ${index+1} in your ${esc(post.status)} post" loading="lazy">${more?`<span class="cite-post-gallery-more" aria-hidden="true">+${remaining}</span>`:''}</button>`;}).join('')}</div>`:'';
  ...
  function render() {
    ...
    root.querySelectorAll('[data-own-image-post]').forEach(button=>button.onclick=()=>{const post=data.own_posts.find(item=>item.id===Number(button.dataset.ownImagePost));if(!post)return;const images=post.images?.length?post.images:post.image_path?[post.image_path]:[];CiteMediaPostCard.openImage(images,`Your ${post.status} post photo`,Number(button.dataset.ownImageIndex));});
