(() => {
  ...
  function reactionPickerMarkup(scope, current, buttonClass = "") {
    ...
    return `<div class="cite-reaction-picker ${buttonClass}" data-reaction-picker data-reaction-scope="${scope}">
      <button class="cite-reaction-current ${active ? "is-reacted" : ""}" type="button" data-reaction-current data-reaction-type="${current || 'like'}" aria-label="${active ? `Remove ${selected.label} reaction` : "React with Like"}" aria-pressed="${active}"><span class="cite-reaction-icon" aria-hidden="true">${active ? selected.icon : reactionDetails.like.icon}</span>${scope === "post" ? `<span>${active ? selected.label : "Like"}</span>` : ""}</button>
  ...
  function create(post, context) {
    ...
    const bindReactionPicker = (picker, comment = null) => {
      ...
      picker.querySelector('[data-reaction-current]').onclick = () => {
        ...
        submitReaction(picker, selected || 'like', !Boolean(selected));
      };
