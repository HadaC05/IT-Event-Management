(() => {
  ...
  function create(post, context) {
    ...
    article.innerHTML = `<header class="flex items-center gap-3 p-4 sm:p-5">${authorAvatar}<div class="min-w-0 flex-1"><div class="flex flex-wrap items-center gap-2"><strong class="truncate text-base">${esc(post.author_name)}</strong>${roleBadge}${devBadge}${post.is_official ? '<span class="rounded bg-[#C6F24E]/40 px-2 py-0.5 text-[10px] font-black text-[#397565]">Official</span>' : ""}</div><div class="mt-0.5 flex flex-wrap gap-2 text-[11px] text-[#121017]/50"><time>${esc(time(post.created_at))}</time>${post.event_title ? `<span>•</span><span class="font-bold text-[#397565]">${esc(post.event_title)}</span>` : ""}</div></div>${menuMarkup(post, viewer, permissions)}</header>`;
